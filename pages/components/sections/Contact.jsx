...
const ContactNew = () => {
    ...
    const [animationDataSendTick, setAnimationDataSendTick] = useState(null);
    const [animationDataContactYoga, setAnimationDataContactYoga] = useState(null);

    // Load file JSON dari /public pakai fetch
    useEffect(() => {
        fetch("/media/lotties/SendTick(Contact).json")
            .then((res) => res.json())
            .then((data) => setAnimationDataSendTick(data))
            .catch((err) => console.error("Error loading Lottie JSON:", err));

        fetch("/media/lotties/FloatingYogaMan(Contact).json")
            .then((res) => res.json())
            .then((data) => setAnimationDataContactYoga(data))
            .catch((err) => console.error("Error loading Lottie JSON:", err));
    }, []);
        ...
        <div>
        {
            ...
            (
                <div className='border-0 border-red-500'>
                    ...
                    <div>
                        <SafeLottie animationData={animationDataSendTick} className="sm1:h-[10rem] md:h-[20rem] grayscale-0 border-0 border-red-500" loop={true}/>
                    </div>    
                    ...
                    <div className='flex justify-around w-full m-auto mt-20 text-white border-0 border-red-500'>
                        <div className='flex border-0 border-red-500 xl:space-x-20 sm1:flex-col xl:flex-row sm1:space-y-2 md:space-y-8 xl:space-y-0'>
                            ...
                            <a href="mailto:rifanaldio@gmail.com" className='border-0 border-red-500'>
                                <div className='flex flex-row space-x-4'>
                                    ...
                                    <div className='leading-[1rem]'>
                                        <p className='sm1:text-md md:text-lg'>rifanaldio@gmail.com</p>
                            ...
                            <div className='flex space-x-4 border-0 border-red-500 sm1:pt-8 md:pt-4 lg:pt-0 md:text-3xl sm1:text-3xl lg:text-2xl'>
                                <Link className='text-blue-400 cursor-pointer rounded-full sm1:p-2 md:p-4 bg-blue-400/20' href="https://github.com/rifanaldio" target="_blank" rel="noreferrer" ><FiGithub /></Link>
                                <a className='text-blue-400 cursor-pointer rounded-full sm1:p-2 md:p-4 bg-blue-400/20' href="https://www.linkedin.com/in/rifan-aldio/" target="_blank" rel="noreferrer"><FaLinkedinIn /></a>
                                <a className='text-blue-400 cursor-pointer rounded-full sm1:p-2 md:p-4 bg-blue-400/20' href="https://drive.google.com/file/d/1-i-ah-HexGqchyXB5ieNgOzleK4Xw60D/view?usp=sharing" target="_blank"><IoDocumentTextOutline /></a>
                            </div>
                        </div>           
                    </div>
                </div>    
            ...
            (
                <>
                    <div className='relative z-10 flex w-10/12 m-auto text-white border-0 border-red-500 lg:flex-row sm1:flex-col mb-10'>
                    ...
                    <div className='relative z-10 flex w-10/12 m-auto text-white lg:flex-row sm1:flex-col lg:space-x-12 lg:justify-between'>
                        <div className='sm1:w-full lg:w-4/5 lg:-mt-20 xl:mt-10'>
                            <div className='p-10 m-auto border-0 xl:-mt-14 sm1:mt-12 md:mt-20 lg:mt-72 border-white/30 rounded-xl bg-blue-400/0 box-shadow-outForm'>    
                                <form onSubmit={submitForm} className='sm1:space-y-8 md:space-y-16 rounded-2xl'>
                                    ...
                                    <div className='flex flex-row md:space-x-10 sm1:space-x-4 relative z-50'>
                                        <button type='submit' className='font-extrabold text-black cursor-pointer bg-blue-400 transform transition ease-in-out duration-300 hover:scale-[1.05] sm1:p-1 md:text-2xl sm1:w-full lg:w-3/4 md:p-4 rounded-xl'>Send</button>
                                        <button type='button' onClick={()=>{
                                            setEmail('')
                                            setName('')
                                            setMessage('')
                                        }} 
                                        className='font-extrabold cursor-pointer text-black bg-blue-400 transform transition ease-in-out duration-300 hover:scale-[1.05] sm1:p-1 md:text-2xl sm1:w-full lg:w-3/4 md:p-4 rounded-xl'>Reset All</button>
                                    </div>
                                </form> 
                            </div>
                        </div>
                        <div className='sm1:w-full lg:w-3/5 lg:mt-96 xl:mt-1 sm1:mt-12 md:mt-28 xl:w-full xl:space-y-32'>

                                    <div className='border-0 top-0 border-orange-500 sm1:w-full lg:w-3/6 xl:w-4/5 lg:ml-32 xl:ml-20'>
                                        <div className='border-0 border-green-500'>
                                            <SafeLottie animationData={animationDataContactYoga} loop={true} width={300} height={200} className='sm1:scale-[2] lg:scale-[4] xl:scale-[2.5]' />
                                        </div>
                                    </div>


                            <div className='flex flex-col w-full border-0 border-blue-500 sm1:mt-4 md:mt-0 md:space-y-4 lg:mt-32'>
                                <div className='flex sm1:flex-col md:flex-row lg:flex-col xl:flex-row'>
                                    <a href="mailto:rifanaldio@gmail.com">
                                        <div className='flex flex-row space-x-4'>
                                    ...
                                    {/* <div className='flex justify-center border-0 border-red-500 sm1:mt-8 lg:ml-0 md:ml-10 xl:ml-auto xl:w-2/5 sm1:w-full md:w-3/5 lg:w-4/5 sm1:justify-between md:text-3xl sm1:text-3xl lg:text-2xl space-x-5'>
                                        <Link className='text-blue-400 cursor-pointer rounded-full sm1:p-2 md:p-4 bg-blue-400/20' href="https://github.com/rifanaldio" target="_blank" rel="noreferrer" ><FiGithub /></Link>
                                        <a className='text-blue-400 cursor-pointer rounded-full sm1:p-2 md:p-4 bg-blue-400/20' href="https://www.linkedin.com/in/rifan-aldio/" target="_blank" rel="noreferrer"><FaLinkedinIn /></a>
                                        <a className='text-blue-400 cursor-pointer rounded-full sm1:p-2 md:p-4 bg-blue-400/20' href="https://drive.google.com/file/d/1-i-ah-HexGqchyXB5ieNgOzleK4Xw60D/view?usp=sharing" target="_blank"><IoDocumentTextOutline /></a>
                                    </div> */}
